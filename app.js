...
// ======================
// CLIENTS ATTITRÉS + RANGS
// ======================
var CLIENTS = [
  {id:'ghost',name:'Ghost',serie:'Power',c1:'#2A1660',c2:'#5B2D9A',col:'#C4B5FD'},
  {id:'assane',name:'Assane',serie:'Lupin',c1:'#0F2A52',c2:'#1B4A8C',col:'#7CBBF0'},
  {id:'annalise',name:'Annalise',serie:'Murder',c1:'#4A1020',c2:'#8B1C3A',col:'#E8889A'},
  {id:'sully',name:'Sully',serie:'Top Boy',c1:'#0C2818',c2:'#1A5C32',col:'#76C48A'},
  {id:'tommy',name:'Tommy',serie:'Peaky Blinders',c1:'#1C1C24',c2:'#3A3A48',col:'#B0B0C0'},
  {id:'walter',name:'Walter',serie:'Breaking Bad',c1:'#0C3818',c2:'#1A6C32',col:'#7ED894'},
  {id:'mareme',name:'Marème',serie:"Maîtresse d'un homme marié",c1:'#3A1A2A',c2:'#7A3550',col:'#E8A0B8'},
  {id:'apash',name:'Apash',serie:'Validé',c1:'#3A1A08',c2:'#7A3510',col:'#FFA04E'},
  {id:'sakho',name:'Sakho',serie:'Sakho & Mangane',c1:'#08303A',c2:'#10606C',col:'#6CD8E0'},
  {id:'wilson',name:'Wilson',serie:'WHO',c1:'#2A2008',c2:'#5A4510',col:'#E8C56A'}
];

var RANKS = [
  {name:'Stagiaire',min:0},
  {name:'Avocat',min:3},
  {name:'Associé',min:10},
  {name:'Bâtonnier',min:25},
  {name:'Légende',min:50}
];

function getRankInfo(duels) {
  duels = duels || 0;
  var cur = RANKS[0], curIdx = 0;
  for (var i = 0; i < RANKS.length; i++) {
    if (duels >= RANKS[i].min) { cur = RANKS[i]; curIdx = i; }
  }
  var next = RANKS[curIdx + 1] || null;
  var progress = 100, remaining = 0;
  if (next) {
    var span = next.min - cur.min;
    var done = duels - cur.min;
    progress = Math.max(2, Math.round(done / span * 100));
    remaining = next.min - duels;
  }
  return { name: cur.name, next: next ? next.name : null, progress: progress, remaining: remaining };
}
...
// ======================
// PORTE OUVERTE — fin de session
// ======================
function showOpenDoor() {
  var rk = getRankInfo(me.duels);
  var el = document.getElementById('dv-opendoor');
  if (!el) return;
  var clName = me.clientName || '';
  var line1 = rk.next ? ('Plus que <b>' + rk.remaining + ' duel' + (rk.remaining > 1 ? 's' : '') + '</b> avant le rang <b>' + rk.next + '</b>') : 'Vous avez atteint le rang <b>Légende</b>';
  var line2 = clName ? ('<b>' + clName + '</b> vous attend sur une nouvelle affaire') : 'De nouvelles affaires vous attendent';
  el.innerHTML =
    '<div style="height:1px;background:rgba(255,255,255,.06);margin:14px 0;"></div>' +
    '<div style="font-size:11px;font-weight:800;color:rgba(255,255,255,.3);letter-spacing:2px;margin-bottom:8px;">VOTRE PROGRESSION</div>' +
    '<div style="font-size:13px;color:rgba(255,255,255,.55);margin-bottom:8px;">' + line1 + '</div>' +
    '<div style="height:6px;border-radius:3px;background:rgba(255,255,255,.06);overflow:hidden;margin-bottom:14px;"><div style="height:100%;width:' + rk.progress + '%;background:linear-gradient(90deg,#A78BFA,#FF7A2E);border-radius:3px;"></div></div>' +
    '<div style="font-size:13px;color:#FF7A2E;font-weight:700;">→ ' + line2 + '</div>';
...
function buildProfile() {
  ...
  var rk = getRankInfo(me.duels);
  var clName = me.clientName || '';
  var clientEl = document.getElementById('p-client-name');
  if (clientEl) clientEl.textContent = clName ? 'Avocat de ' + clName : 'Avocat indépendant';
  var rankEl = document.getElementById('p-rank-name');
  if (rankEl) rankEl.textContent = rk.name;
  var barEl = document.getElementById('p-rank-bar');
  if (barEl) barEl.style.width = rk.progress + '%';
  var nextEl = document.getElementById('p-rank-next');
  if (nextEl) nextEl.textContent = rk.next ? (rk.remaining + ' duel' + (rk.remaining > 1 ? 's' : '') + ' avant ' + rk.next) : 'Rang maximum atteint';
  // Avatar client
  var clAv = document.getElementById('p-client-av');
  if (clAv) {
    var cl = CLIENTS.filter(function(c){ return c.id === me.clientId; })[0];
    if (cl) { clAv.textContent = cl.name.substring(0,2).toUpperCase(); clAv.style.background = 'linear-gradient(135deg,' + cl.c1 + ',' + cl.c2 + ')'; }
    else { clAv.textContent = '?'; }
  }
...
function initMainApp() {
  // Enregistrer le service worker (sans bloquer la navigation)
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./sw.js').catch(function(err) { console.log('SW:', err); });
  }
  // RÈGLE: on n'ouvre JAMAIS sur une page intermédiaire (client-select, onboarding...).
  // Un utilisateur n'arrive sur home QUE s'il a un compte complet (nom + email + onboarded).
  // Sinon → toujours l'écran d'accueil (splash) avec connexion / inscription.
  try {
    var s = localStorage.getItem('dbrief_user');
    if (s) {
      var u = JSON.parse(s);
      if (u && u.name && u.email && u.onboarded) {
        Object.assign(me, u);
