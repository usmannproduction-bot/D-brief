...
var ALL_CASES = {
  ...
  faille_gav: F('La Garde à Vue', 'La Faille', "Un suspect est interpellé. Repérez le vice de procédure dans le déroulé de sa garde à vue.", "Interpellé à 22h, le suspect est placé en garde à vue. Voici comment les choses se sont déroulées. À vous de trouver ce qui rend toute la procédure attaquable.", [
    {t: "Il a été informé de son droit de garder le silence dès son arrivée", ok: false},
    {t: "Un médecin l'a examiné à sa demande dans l'heure", ok: false},
    {t: "Sa garde à vue a duré 72h sans présentation à un magistrat", ok: true},
    {t: "Son avocat a pu s'entretenir avec lui en privé", ok: false}
  ], "La faille : en droit commun, la garde à vue ne peut dépasser 48h sans présentation à un magistrat qui doit la prolonger. 72h sans contrôle judiciaire rend toute la procédure nulle."),
  faille_perq: F('La Perquisition', 'La Faille', "Les enquêteurs perquisitionnent un domicile. Trouvez l'anomalie qui invalide la saisie.", "Dans le cadre d'une enquête, les agents se présentent au domicile du suspect pour une perquisition. Tout semble en règle… sauf un détail qui rend les preuves irrecevables.", [
    {t: "Ils ont présenté un mandat signé par un juge", ok: false},
    {t: "La perquisition a commencé à 5h du matin", ok: true},
    {t: "Deux témoins indépendants étaient présents", ok: false},
    {t: "Un procès-verbal détaillé a été dressé sur place", ok: false}
  ], "La faille : les perquisitions sont en principe interdites entre 21h et 6h. Commencer à 5h du matin, hors flagrance ou autorisation spéciale, rend la saisie irrecevable."),
  faille_juge: F('Le Procès', 'La Faille', "Au tribunal, l'accusation présente son dossier. Une violation du droit à un procès équitable s'est glissée ici.", "Le procès s'ouvre. L'accusation déroule ses preuves, les témoins défilent. Mais une règle fondamentale du procès équitable a été bafouée. Laquelle ?", [
    ...
    {t: "Le juge d'instruction de l'affaire présidait aussi le procès", ok: true},
    {t: "L'accusé était assisté de son avocat tout au long", ok: false}
  ], "La faille : le principe de séparation des fonctions interdit au juge d'instruction (qui a mené l'enquête) de juger ensuite l'affaire. C'est une atteinte directe à l'impartialité du tribunal.")
...
function buildHome() {
  ...
  sorted.forEach(function(k, idx) {
    var c = CASES[k];
    var isDDJ = (k === ddj);
    var locked = (me.email && !me.sigDone && me.clientSig && k !== me.clientSig);
...
// Lance l'affaire signature du client (porte d'entrée obligatoire)
function startSignatureCase() {
  var sig = me.clientSig;
  if (!sig || !CASES[sig]) { buildHome(); go('home'); return; }
  joinDuelAs(sig, 'defense');
}
...
function pickFaille(i) {
  ...
  if (me.email && me.clientSig && curCase === me.clientSig && !me.sigDone) {
    me.sigDone = true;
