...
// ======================
var CABINETS = {
  langstone:{name:'Langstone & Cole',c1:'#0F2A52',c2:'#1B4A8C',col:'#7CBBF0',clients:['Annalise','Harvey','Walter','Mangane','Tommy','Barbe Noire','Brochant','Icy','Sarah']},
  zelder:{name:'Zelder-Carvani',c1:'#3A1A08',c2:'#7A3510',col:'#E8C56A',clients:['Michaela','Jesse','Hubert','Dadié','Roman','Candice','William','Dialika','Éric']},
  usman:{name:'Usman-Khanna',c1:'#2A1660',c2:'#5B2D9A',col:'#C4B5FD',clients:['Assane','Ghost','Aaron','Sakho','Wilson','Ebony','Kemi','Le Fils','Le Peuple']},
  adam:{name:'Adam-Idrissou',c1:'#0C2818',c2:'#1A5C32',col:'#76C48A',clients:['Apash','Sully','Marème','Arthur','Tag','Pignon','Bloom',"L'Oncle",'Le Président']}
};
var CAB_AFF = {
  langstone:[{id:'annalise1',mc:'Annalise',oc:'Michaela',ocab:'Zelder-Carvani',sr:'Murder'},{id:'walter1',mc:'Walter',oc:'Jesse',ocab:'Zelder-Carvani',sr:'Breaking Bad'},{id:'nuitdesrois1',mc:'Barbe Noire',oc:'Roman',ocab:'Zelder-Carvani',sr:'La Nuit des Rois'},{id:'dadie1',mc:'Harvey',oc:'Dadié',ocab:'Usman-Khanna',sr:'Philosophie'},{id:'blood1',mc:'Sarah',oc:'Kemi',ocab:'Usman-Khanna',sr:'Blood Sisters'},{id:'sakho1',mc:'Mangane',oc:'Sakho',ocab:'Usman-Khanna',sr:'Sakho & Mangane'},{id:'tommy1',mc:'Tommy',oc:'Arthur',ocab:'Adam-Idrissou',sr:'Peaky Blinders'},{id:'dinercon1',mc:'Brochant',oc:'Pignon',ocab:'Adam-Idrissou',sr:'Le Dîner de Cons'},{id:'bloom1',mc:'Icy',oc:'Bloom',ocab:'Adam-Idrissou',sr:'Les Winx'}],
  zelder:[{id:'annalise1',mc:'Michaela',oc:'Annalise',ocab:'Langstone & Cole',sr:'Murder'},{id:'walter1',mc:'Jesse',oc:'Walter',ocab:'Langstone & Cole',sr:'Breaking Bad'},{id:'nuitdesrois1',mc:'Roman',oc:'Barbe Noire',ocab:'Langstone & Cole',sr:'La Nuit des Rois'},{id:'lupin1',mc:'Hubert',oc:'Assane',ocab:'Usman-Khanna',sr:'Lupin'},{id:'nemesis1',mc:'Candice',oc:'Ebony',ocab:'Usman-Khanna',sr:'Nemesis'},{id:'who2',mc:'Le Groupe',oc:'Wilson',ocab:'Usman-Khanna',sr:'WHO'},{id:'valide1',mc:'William',oc:'Apash',ocab:'Adam-Idrissou',sr:'Validé'},{id:'maitresse1',mc:'Dialika',oc:'Marème',ocab:'Adam-Idrissou',sr:"Maîtresse"},{id:'tag1',mc:'Éric',oc:'Tag',ocab:'Adam-Idrissou',sr:'Foot 2 Rue'}],
  usman:[{id:'dadie1',mc:'Dadié',oc:'Harvey',ocab:'Langstone & Cole',sr:'Philosophie'},{id:'blood1',mc:'Kemi',oc:'Sarah',ocab:'Langstone & Cole',sr:'Blood Sisters'},{id:'sakho1',mc:'Sakho',oc:'Mangane',ocab:'Langstone & Cole',sr:'Sakho & Mangane'},{id:'lupin1',mc:'Assane',oc:'Hubert',ocab:'Zelder-Carvani',sr:'Lupin'},{id:'nemesis1',mc:'Ebony',oc:'Candice',ocab:'Zelder-Carvani',sr:'Nemesis'},{id:'who2',mc:'Wilson',oc:'Le Groupe',ocab:'Zelder-Carvani',sr:'WHO'},{id:'topboy1',mc:'Aaron',oc:'Sully',ocab:'Adam-Idrissou',sr:'Top Boy'},{id:'heritage1',mc:'Le Fils',oc:"L'Oncle",ocab:'Adam-Idrissou',sr:'Héritage'},{id:'mandat1',mc:'Le Peuple',oc:'Le Président',ocab:'Adam-Idrissou',sr:'Philosophie & Pouvoir'}],
  adam:[{id:'tommy1',mc:'Arthur',oc:'Tommy',ocab:'Langstone & Cole',sr:'Peaky Blinders'},{id:'dinercon1',mc:'Pignon',oc:'Brochant',ocab:'Langstone & Cole',sr:'Le Dîner de Cons'},{id:'bloom1',mc:'Bloom',oc:'Icy',ocab:'Langstone & Cole',sr:'Les Winx'},{id:'valide1',mc:'Apash',oc:'William',ocab:'Zelder-Carvani',sr:'Validé'},{id:'maitresse1',mc:'Marème',oc:'Dialika',ocab:'Zelder-Carvani',sr:"Maîtresse"},{id:'tag1',mc:'Tag',oc:'Éric',ocab:'Zelder-Carvani',sr:'Foot 2 Rue'},{id:'topboy1',mc:'Sully',oc:'Aaron',ocab:'Usman-Khanna',sr:'Top Boy'},{id:'heritage1',mc:"L'Oncle",oc:'Le Fils',ocab:'Usman-Khanna',sr:'Héritage'},{id:'mandat1',mc:'Le Président',oc:'Le Peuple',ocab:'Usman-Khanna',sr:'Philosophie & Pouvoir'}]
};
var selectedCabinet='';var curAffaire='';var salonTab='notifs';
...
function switchTab(id) {
  if (currentRoom && id !== 'home' && id !== 'verdicts' && id !== 'profil' && id !== 'cabinet') { leaveDuel(); }
  go(id);
  if (id === 'profil') { buildProfile(); loadProfile(); }
  if (id === 'verdicts') loadPublicDuels();
  if (id === 'home') loadFeed();
  if (id === 'cabinet') { buildSalon(); }
}
...
function buildHome() {
  ...
  // Construire l'ordre : DDJ en premier, puis favoris, puis reste
  var favKeys = [];
  var restKeys = [];
  caseKeys.forEach(function(k) {
    if (k === ddj) return; // DDJ géré séparément
    if (fav.length > 0 && fav.indexOf(CASES[k].t) > -1) {
      favKeys.push(k);
    } else {
      restKeys.push(k);
    }
  ...
  var sorted = [];
  if (CASES[ddj]) sorted.push(ddj);
  sorted = sorted.concat(favKeys).concat(restKeys);
  ...
  sorted.forEach(function(k, idx) {
    ...
    // Badge Duel du Jour
    var ddjBadge = isDDJ
      ? '<span style="font-size:10px;padding:3px 12px;background:linear-gradient(135deg,rgba(255,122,46,.2),rgba(167,139,250,.2));border:1px solid rgba(255,122,46,.4);border-radius:10px;color:#FF7A2E;font-weight:800;letter-spacing:1px;">🔥 DUEL DU JOUR</span>'
      : '';

    // Indicateur "autres affaires" sur la dernière carte du DDJ
    var swipeHint = isDDJ
      ? '<div style="position:absolute;bottom:90px;left:50%;transform:translateX(-50%);font-size:11px;color:rgba(255,255,255,.15);white-space:nowrap;">D\'autres affaires vous attendent ↓</div>'
      : '';

    card.innerHTML =
      '<div style="position:absolute;top:30%;left:50%;transform:translateX(-50%);width:280px;height:280px;background:radial-gradient(circle,' + (isDDJ ? 'rgba(255,122,46,.12)' : 'rgba(167,139,250,.1)') + ',transparent 70%);pointer-events:none"></div>' +
...
function leaveDuel() {
  ...
  curCase = '';
  myRole = '';
  if (!me.onboarded) { buildSeriesGrid(); go('series-select'); } else { go('home'); }
}
  ...
  if (code) {
    ...
    waitDb(function() {
      window.db.ref('cabinets').orderByChild('code').equalTo(code).once('value', function(snap) {
        ...
        err.style.display = 'none'; saveSession(); go('onboarding');
      });
    });
    return;
  }
...
function logout() { localStorage.removeItem('dbrief_user'); location.reload(); }
...
function handleUrlRoom() {
  ...
  if (CASES[roomCaseId]) {
    ...
    if (window.db) {
      ...
      window.db.ref('rooms/' + currentRoom + '/roles').on('value', function(snap2) {
        var r = snap2.val() || {};
        if (r.defense) { document.getElementById('dw-av-def').textContent = (r.defense.name || '?').substring(0, 2).toUpperCase(); document.getElementById('dw-av-def').style.borderStyle = 'solid'; document.getElementById('dw-name-def').textContent = r.defense.name || ''; }
        if (r.accusation) { document.getElementById('dw-av-acc').textContent = (r.accusation.name || '?').substring(0, 2).toUpperCase(); document.getElementById('dw-av-acc').style.borderStyle = 'solid'; document.getElementById('dw-av-acc').style.borderColor = '#FF4757'; document.getElementById('dw-av-acc').style.color = '#FF4757'; document.getElementById('dw-name-acc').textContent = r.accusation.name || ''; }
        document.getElementById('dw-count').textContent = ((r.defense ? 1 : 0) + (r.accusation ? 1 : 0)) + ' personne(s)';
        if (r.defense && r.accusation) { document.getElementById('dw-status').textContent = 'Duel prêt!'; setTimeout(startDuel, 2000); }
      });
    }
  } else { go('home'); }
}
...
// CABINET SELECT
// ======================
function buildCabinetSelect(){
  var wrap=document.getElementById('cab-cards');if(!wrap)return;wrap.innerHTML='';
  ['langstone','zelder','usman','adam'].forEach(function(id){
    var cab=CABINETS[id];var card=document.createElement('div');
    card.className='cab-card';card.id='cc-'+id;card.onclick=function(){pickCabinet(id);};
    var chips=cab.clients.slice(0,5).map(function(c){return'<span class="cab-chip">'+c+'</span>';}).join('')+'<span class="cab-chip" style="opacity:.4;">+'+(cab.clients.length-5)+'</span>';
    card.innerHTML='<div class="cab-top-blk" style="background:linear-gradient(135deg,'+cab.c1+','+cab.c2+');"><div class="cab-chk" id="chk-'+id+'"><svg class="cab-chk-svg" width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4L3.5 6.5L9 1" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div><div class="cab-pre">Cabinet</div><div class="cab-nm">'+cab.name+'</div></div></div><div class="cab-bot"><div class="mbr-inf"><b id="mc-'+id+'">0</b> / 10 membres</div><div class="sec-lbl-sm">Membres</div><div class="cab-mbrs" id="mv-'+id+'"><div class="cab-av empty">+</div></div><div class="sec-lbl-sm" style="margin-top:10px;">Clients</div><div class="cab-clients">'+chips+'</div></div>';
    wrap.appendChild(card);
    if(window.db){window.db.ref('cabinets_v2/'+id+'/members').once('value',function(snap){var m=snap.val()||{};var k=Object.keys(m);var mc=document.getElementById('mc-'+id);if(mc)mc.textContent=k.length;var mv=document.getElementById('mv-'+id);if(!mv)return;mv.innerHTML='';k.slice(0,8).forEach(function(u){var ini=(m[u].name||'?').substring(0,2).toUpperCase();mv.innerHTML+='<div class="cab-av" style="background:'+cab.c2+';">'+ini+'</div>';});if(k.length<10)mv.innerHTML+='<div class="cab-av empty">+</div>';});}
  });
}

function pickCabinet(id){
  ['langstone','zelder','usman','adam'].forEach(function(x){var e=document.getElementById('cc-'+x);if(e)e.classList.remove('sel');});
  selectedCabinet=id;var el=document.getElementById('cc-'+id);if(el)el.classList.add('sel');
  var btn=document.getElementById('cab-confirm-btn');if(btn){btn.style.opacity='1';btn.style.pointerEvents='all';btn.textContent='Rejoindre '+CABINETS[id].name;}
}

function confirmCabinet(){
  if(!selectedCabinet)return;var cab=CABINETS[selectedCabinet];
  me.cabId=selectedCabinet;me.cabName=cab.name;saveSession();
  if(window.db&&me.email){window.db.ref('cabinets_v2/'+selectedCabinet+'/members/'+myUid).set({name:me.name,joined:Date.now()});window.db.ref('users/'+myUid+'/cabId').set(selectedCabinet);window.db.ref('users/'+myUid+'/cabName').set(cab.name);}
  if(!me.email){var el=document.getElementById('conv-cab-name');if(el)el.textContent=cab.name;go('cabinet-convert');return;}
  buildHome();buildProfile();go('home');
}

function skipCabinet(){me.onboarded=true;saveSession();buildHome();buildProfile();go('home');}

// ======================
// SALON CABINET
// ======================
function buildSalon(){
  if(!me.cabId||!CABINETS[me.cabId]){buildCabinetSelect();go('cabinet-select');return;}
  var cab=CABINETS[me.cabId];
  var hdr=document.getElementById('salon-hdr-bg');if(hdr)hdr.style.background='radial-gradient(ellipse 120% 100% at 10% 0%,'+cab.c2+'99 0%,transparent 55%),linear-gradient(160deg,'+cab.c1+' 0%,#06060C 70%)';
  var nm=document.getElementById('salon-cab-name');if(nm)nm.textContent=cab.name;
  var av=document.getElementById('salon-my-av');if(av){av.textContent=(me.name||'?').substring(0,2).toUpperCase();av.style.background='linear-gradient(135deg,'+cab.c2+','+cab.col+')';}
  loadSalonMembers();switchSalonTab('notifs');
}

function loadSalonMembers(){
  if(!window.db||!me.cabId)return;
  window.db.ref('cabinets_v2/'+me.cabId+'/members').once('value',function(snap){
    var mbrs=snap.val()||{};var k=Object.keys(mbrs);
    var mc=document.getElementById('salon-mbr-count');if(mc)mc.textContent=k.length+' / 10 membres';
    var strip=document.getElementById('salon-mbrs-strip');if(!strip)return;
    var cab=CABINETS[me.cabId];strip.innerHTML='';
    k.slice(0,5).forEach(function(uid){var m=mbrs[uid];var ini=(m.name||'?').substring(0,2).toUpperCase();var brd=uid===myUid?'border:2px solid rgba(255,255,255,.6);':'';strip.innerHTML+='<div class="s-mbr-av" style="background:linear-gradient(135deg,'+cab.c2+','+cab.col+');'+brd+'">'+ini+'</div>';});
  });
}

function switchSalonTab(tab){
  salonTab=tab;
  document.getElementById('stab-notifs').classList.toggle('on',tab==='notifs');
  document.getElementById('stab-aff').classList.toggle('on',tab==='aff');
  document.getElementById('sv-notifs').style.display=tab==='notifs'?'block':'none';
  document.getElementById('sv-aff').style.display=tab==='aff'?'block':'none';
  if(tab==='aff')buildSalonAffaires();
}

function buildSalonAffaires(){
  var wrap=document.getElementById('salon-aff-list');if(!wrap||!me.cabId||!CAB_AFF[me.cabId])return;
  wrap.innerHTML='';var cab=CABINETS[me.cabId];
  CAB_AFF[me.cabId].forEach(function(a){
    var card=document.createElement('div');card.className='aff-crd'+(curAffaire===a.id?' on':'');card.id='af-'+a.id;card.onclick=function(){pickAffaire(a.id);};
    card.innerHTML='<div class="aff-strip"></div><div class="aff-inn"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;"><div class="aff-sr">'+a.sr+'</div><div class="aff-chk" id="achk-'+a.id+'"><svg class="aff-chk-svg" width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4L3.5 6.5L9 1" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg></div></div><div class="vs-blk"><div class="vs-fgt"><div class="vs-fgt-role" style="color:'+cab.col+';">Votre client</div><div class="vs-fgt-name">'+a.mc+'</div></div><div class="vs-sep"><div class="vs-ln"></div><div class="vs-tx">VS</div><div class="vs-ln"></div></div><div class="vs-fgt opp" style="text-align:right;"><div class="vs-fgt-role">Adversaire</div><div class="vs-fgt-name">'+a.oc+'</div></div></div><div class="aff-tags"><div class="aff-tag aff-tag-self">'+me.cabName+'</div><div class="aff-tag aff-tag-opp">'+a.ocab+'</div></div></div>';
    wrap.appendChild(card);
  });
}

function pickAffaire(id){
  var prev=document.getElementById('af-'+curAffaire);if(prev)prev.classList.remove('on');
  curAffaire=(curAffaire===id)?'':id;
  if(curAffaire){var el=document.getElementById('af-'+curAffaire);if(el)el.classList.add('on');if(window.db&&me.cabId)window.db.ref('cabinets_v2/'+me.cabId+'/active_affaires/'+myUid).set({affaireId:curAffaire,name:me.name,ts:Date.now()});}
}
